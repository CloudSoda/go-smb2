...
type Negotiator struct {
	...
	// SigningAlgorithms lists the signing algorithms to advertise in the
	// SMB2_SIGNING_CAPABILITIES negotiate context (SMB 3.1.1 only), in
	// preference order (most preferred first). Allowed values are
	// [SigningAESGMAC] and [SigningAESCMAC]. If nil, both are offered with
	// AES-GMAC preferred.
...
func (n *Negotiator) makeRequest() (*smb2.NegotiateRequest, error) {
	...
	signingAlgs := clientSigningAlgorithms
	if n.SigningAlgorithms != nil {
		signingAlgs = n.SigningAlgorithms
	}
	...
	if n.SpecifiedDialect != smb2.UnknownSMB {
		...
		case smb2.SMB311:
			...
			sc := &smb2.SigningContext{
				SigningAlgorithms: signingAlgs,
			}

			req.Contexts = append(req.Contexts, hc, cc, sc)
		default:
...
retry:
	...
	conn.signingAlgorithmId = smb2.SMB2_SIGNING_AES_CMAC // default; may be overridden by SMB2_SIGNING_CAPABILITIES context below
	...
	for count := r.NegotiateContextCount(); count > 0; count-- {
		...
		case smb2.SMB2_SIGNING_CAPABILITIES:
			...
			if d.IsInvalid() {
				return nil, &InvalidResponseError{"broken signing context data format"}
			}

			algs := d.SigningAlgorithms()

			if len(algs) != 1 {
				return nil, &InvalidResponseError{"multiple signing algorithms"}
			}

			conn.signingAlgorithmId = algs[0]

			switch conn.signingAlgorithmId {
			case smb2.SMB2_SIGNING_AES_CMAC:
			case smb2.SMB2_SIGNING_AES_GMAC:
			default:
				return nil, &InvalidResponseError{"unknown signing algorithm"}
			}
		default:
