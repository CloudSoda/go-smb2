...
type Negotiator struct {
	...
	// MinDialect and MaxDialect bound the dialects offered to the server.
	// Zero means unbounded in that direction, so leaving both alone offers
	// everything the client supports, as before.
	//
	// A bound rather than a list keeps this forward compatible: refusing
	// everything below SMB300 keeps refusing it when a later dialect is
	// added, where a list would have to be revisited.
	//
	// SpecifiedDialect pins exactly one dialect and takes precedence over
	// both; it is not checked against them.
	MinDialect uint16
	MaxDialect uint16
}

// dialects returns the dialects to offer, most preferred first, after
// applying MinDialect and MaxDialect.
func (n *Negotiator) dialects() ([]uint16, error) {
	if n.MinDialect != 0 && !isKnownDialect(n.MinDialect) {
		return nil, &InternalError{fmt.Sprintf("unknown min dialect: 0x%03x", n.MinDialect)}
	}
	if n.MaxDialect != 0 && !isKnownDialect(n.MaxDialect) {
		return nil, &InternalError{fmt.Sprintf("unknown max dialect: 0x%03x", n.MaxDialect)}
	}
	if n.MinDialect != 0 && n.MaxDialect != 0 && n.MinDialect > n.MaxDialect {
		return nil, &InternalError{fmt.Sprintf("min dialect 0x%03x is above max dialect 0x%03x", n.MinDialect, n.MaxDialect)}
	}

	if n.MinDialect == 0 && n.MaxDialect == 0 {
		return clientDialects, nil
	}

	dialects := make([]uint16, 0, len(clientDialects))
	for _, d := range clientDialects {
		if n.MinDialect != 0 && d < n.MinDialect {
			continue
		}
		if n.MaxDialect != 0 && d > n.MaxDialect {
			continue
		}
		dialects = append(dialects, d)
	}
	if len(dialects) == 0 {
		return nil, &InternalError{fmt.Sprintf("no supported dialect between 0x%03x and 0x%03x", n.MinDialect, n.MaxDialect)}
	}
	return dialects, nil
}

func isKnownDialect(d uint16) bool {
	for _, known := range clientDialects {
		if d == known {
			return true
		}
	}
...
func containsDialect(dialects []uint16, d uint16) bool {
	for _, have := range dialects {
		if have == d {
			return true
		}
	}
...
func (n *Negotiator) makeRequest() (*smb2.NegotiateRequest, error) {
	...
	if n.SpecifiedDialect != smb2.UnknownSMB {
		...
		dialects, err := n.dialects()
		if err != nil {
			return nil, err
		}
		req.Dialects = dialects

		// The negotiate contexts are an SMB 3.1.1 construct. Sending them
		// when 3.1.1 is not on offer says nothing the server can use.
		if containsDialect(dialects, smb2.SMB311) {
			hc := &smb2.HashContext{
				HashAlgorithms: clientHashAlgorithms,
				HashSalt:       make([]byte, 32),
			}
...
retry:
	...
	if r.DialectRevision() == smb2.SMB2 {
		// The server answered the wildcard revision, which means "ask me
		// again for a 2.x dialect". Honour the caller's bounds rather than
		// quietly dropping to 2.1 below a floor they set.
		if n.MinDialect != 0 && smb2.SMB210 < n.MinDialect {
			return nil, &InvalidResponseError{fmt.Sprintf("server offered only SMB 2.1, below the requested minimum 0x%03x", n.MinDialect)}
		}
