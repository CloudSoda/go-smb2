...
type Negotiator struct {
	// RequireMessageSigning, when true, sets SMB2_NEGOTIATE_SIGNING_REQUIRED in
	// the NEGOTIATE request and causes the client to reject any response that is
	// not signed. When false, signing is advertised as supported
	// (SMB2_NEGOTIATE_SIGNING_ENABLED) but not enforced.
	RequireMessageSigning bool

	// RequireMessageEncryption, when true, causes Dial to fail if the server
	// does not support SMB encryption (SMB2_GLOBAL_CAP_ENCRYPTION in the
	// NEGOTIATE response), if the completed session does not carry
	// SMB2_SESSION_FLAG_ENCRYPT_DATA, or if a mounted share carries neither
	// the session flag nor SMB2_SHAREFLAG_ENCRYPT_DATA.
	//
	// Encryption requires SMB 3.0 or later. When this flag is set, the
	// advertised dialect list is automatically restricted to SMB 3.x, so
	// setting MinDialect separately is not required unless a higher minimum
	// is desired. If MaxDialect is set to a value below SMB 3.0, Dial returns
	// an error immediately.
	//
	// When encryption takes effect: NEGOTIATE and SESSION_SETUP messages are
	// always exchanged in plaintext because the session key — from which
	// encryption keys are derived — does not exist until SESSION_SETUP
	// completes. Credentials are protected by the authentication sub-protocol
	// (NTLMv2 challenge-response or Kerberos), not by SMB-layer encryption.
	// SMB encryption applies to TREE_CONNECT and all subsequent I/O.
	//
	// WARNING: Windows Server and Windows desktop SMB shares ship with
	// encryption DISABLED by default. Samba also defaults to no encryption
	// unless explicitly configured with 'smb encrypt = required'. Without
	// setting this flag the client will silently connect in plaintext to any
	// out-of-the-box Windows or Samba endpoint, even when the negotiated
	// dialect is SMB 3.x. This is the common case, not the exception.
	RequireMessageEncryption bool

	// ClientGuid is the 16-byte client identifier sent in the NEGOTIATE request.
	// If left as the zero value, a random GUID is generated using crypto/rand on
	// every call to Dial.
	ClientGuid [16]byte

	// MinDialect, when non-zero, is the minimum dialect version the client will
	// accept. The negotiation fails if the server selects a dialect below this
	// value. Use the DialectSMBxxx constants (e.g. DialectSMB300).
	MinDialect uint16

	// MaxDialect, when non-zero, is the maximum dialect version the client will
	// offer. Dialects above this value are removed from the advertised list.
	// Use the DialectSMBxxx constants (e.g. DialectSMB311).
	MaxDialect uint16

	// Ciphers is the ordered list of cipher IDs to advertise in the
	// SMB2_ENCRYPTION_CAPABILITIES negotiate context for SMB 3.1.1 connections.
	// The server picks the first ID it also supports, so put the most-preferred
	// cipher first.
	//
	// Use the CipherAESxxx constants (e.g. CipherAES128GCM, CipherAES128CCM).
	// A nil or empty slice falls back to the library default
	// [CipherAES128GCM, CipherAES128CCM].
	//
	// This field is only consulted for SMB 3.1.1; it is ignored for SMB 3.0 and
	// earlier dialects, which do not use the SMB2_ENCRYPTION_CAPABILITIES context.
	Ciphers []uint16

	// specifiedDialect is an internal field set only by the SMB2-wildcard retry
	// path in negotiate() (server responded with dialect 0x2FF, client retries
	// pinned to SMB 2.1). Callers should use MinDialect/MaxDialect instead.
...
func (n *Negotiator) makeRequest() (*smb2.NegotiateRequest, error) {
	...
	if n.specifiedDialect != smb2.UnknownSMB {
		...
		// Build the dialect list from clientDialects, filtered by MinDialect/MaxDialect.
		//
		// When RequireMessageEncryption is set, SMB 3.0 or later is mandatory
		// (SMB 2.x has no encryption). Fail early if MaxDialect excludes all SMB 3.x
		// dialects; otherwise raise the effective minimum to SMB 3.0 automatically
		// so the caller does not have to set MinDialect explicitly.
		effectiveMin := n.MinDialect
		if n.RequireMessageEncryption {
			if n.MaxDialect != smb2.UnknownSMB && n.MaxDialect < smb2.SMB300 {
				return nil, &InternalError{"RequireMessageEncryption is set but MaxDialect is below SMB 3.0, which does not support encryption"}
			}
			if effectiveMin < smb2.SMB300 {
				effectiveMin = smb2.SMB300
			}
		}

		dialects := clientDialects
		if effectiveMin != smb2.UnknownSMB || n.MaxDialect != smb2.UnknownSMB {
			filtered := dialects[:0:0]
			for _, d := range dialects {
				if effectiveMin != smb2.UnknownSMB && d < effectiveMin {
					continue
				}
				if n.MaxDialect != smb2.UnknownSMB && d > n.MaxDialect {
					continue
				}
				filtered = append(filtered, d)
			}
			if len(filtered) == 0 {
				return nil, &InternalError{"no supported dialects in MinDialect–MaxDialect range"}
			}
			dialects = filtered
		}
...
retry:
	...
	if n.MinDialect != smb2.UnknownSMB && r.DialectRevision() < n.MinDialect {
		return nil, &InvalidResponseError{"server dialect below required minimum"}
	}

	if n.MaxDialect != smb2.UnknownSMB && r.DialectRevision() > n.MaxDialect {
		return nil, &InvalidResponseError{"server dialect above required maximum"}
	}
	...
	if n.RequireMessageEncryption && conn.capabilities&smb2.SMB2_GLOBAL_CAP_ENCRYPTION == 0 {
		return nil, &InvalidResponseError{"encryption required but server does not support it"}
	}
	conn.requireEncryption = n.RequireMessageEncryption
	...
	if conn.requireEncryption && conn.cipherId == 0 {
		return nil, &InvalidResponseError{"encryption required but server did not negotiate a cipher"}
	}
