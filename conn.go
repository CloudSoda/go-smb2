...
type Negotiator struct {
	...
	// Ciphers lists the SMB 3.x encryption cipher algorithms the client is
	// willing to use, in preference order (most preferred first). Use the
	// CipherAES* constants exported from this package. When nil or empty,
	// the compiled-in default list is used (AES-256-GCM, AES-256-CCM,
	// AES-128-GCM, AES-128-CCM). Only meaningful for SMB 3.1.1; ignored
	// for older dialects.
	Ciphers []uint16
}

func (n *Negotiator) effectiveCiphers() ([]uint16, error) {
	if len(n.Ciphers) == 0 {
		return clientCiphers, nil
	}
	for _, c := range n.Ciphers {
		switch c {
		case smb2.AES128CCM, smb2.AES128GCM, smb2.AES256CCM, smb2.AES256GCM:
		default:
			return nil, &InternalError{fmt.Sprintf("unsupported cipher 0x%04x in Negotiator.Ciphers", c)}
		}
	}
	return n.Ciphers, nil
}

func (n *Negotiator) makeRequest() (*smb2.NegotiateRequest, error) {
	ciphers, err := n.effectiveCiphers()
	if err != nil {
		return nil, err
	}
